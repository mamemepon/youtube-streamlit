...
# --- 検索ボックス ---
search_word = st.text_input("🔍 店名またはカテゴリーまたはエリアで検索", "")
...
if search_word:
    df_filtered = df[df["name"].str.contains(search_word, case=False, na=False) |
                     ...
                     df["area"].str.contains(search_word, case=False, na=False)]
else:
    df_filtered = df
...
# --- 一覧表示 ---
st.subheader("📍 行ってみたとこ一覧")

if df_filtered.empty:
    st.info("登録されたお店はありません。")
else:
    for _, row in df_filtered.iterrows():
        with st.expander(f"{row['name']} | {row['category']}| {row['area']} |　クオリティ⭐{row['rating1']} | コスパ⭐{row['rating2']} | 居心地の良さ⭐{row['rating3']} | 行って後悔しない度⭐{row['rating4']}"):
